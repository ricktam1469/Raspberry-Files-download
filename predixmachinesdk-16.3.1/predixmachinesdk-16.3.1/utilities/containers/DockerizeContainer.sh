...
MACHINE_PATH=
DOCKER_HOST=
DOCKER_FILE_NAME=Dockerfile-predixdtr-alpine
ARCHITECTURE=x86_64
CONTAINER_NAME='default'
TAR_NAME=

# DO NOT CHANGE THE NAME OF THIS VARIABLE. IT TAGS PREDIX MACHINE WITH THIS NAME. THE BOOTSTRAP REQUIRES THIS.
DOCKER_IMAGE_NAME=predixmachine
DOCKER_FTP_PROXY=
DOCKER_HTTP_PROXY=
DOCKER_HTTPS_PROXY=
DOCKER_NO_PROXY=
...
PrintUsage () {
    ...
    echo "   sh DockerizeContainer.sh -m ~/MyPredixMachine --docker_host default --arch x86_64 --container_name agent --tar_name PredixMachine --http_proxy http://proxy-src.research.ge.com:8080 --https_proxy http://proxy-src.research.ge.com:8080 --no_proxy localhost,127.0.0.1,*.ge.com"
    echo ""
    echo "REQUIRED:"
...
DockerizeMachine () {
    ...
    COMMAND="docker build -f \"$DOCKER_FILE_NAME-$ARCHITECTURE\" -t \"${DOCKER_IMAGE_NAME}-${ARCHITECTURE}:${MACHINE_BUILD_VERSION}-${CONTAINER_NAME}\""
    ...
    docker save -o ../"$TAR_FILE" "${DOCKER_IMAGE_NAME}-${ARCHITECTURE}:${MACHINE_BUILD_VERSION}-$CONTAINER_NAME"
    if [ ! -e ../"$TAR_FILE" ]; then
