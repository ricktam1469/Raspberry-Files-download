...
public class HttpClientSample
{
    ...
    /**
     * This injected factory creates private instances of IHttpClient that supports Predix Cloud authenticated communication.
     * NOTE: This factory will only be injected when configuration file
     * <PredixMachine>/configuration/machine.com.ge.dspmicro.predixcloud.identity.config
     * is populated with proper settings through enrollment.
     */
    ...
    public void activate(ComponentContext ctx)
    {
        ...
        // The REST server may not have completed start up yet. Best practice is to do this in a thread since you don't know how long it will take for the URL
        // to become active.
        Runnable runner = new Runnable()
        {
            ...
            private void callWithHttpClient()
            {
                ...
                for (int ii = 0; ii < maxTries; ii++)
                {
                    try
                    {
                        ...
                        {
                            _logger.info(
                                    "HttpClientSample: GET success: " + new String(httpResponse.getContent()).trim());
                            return;
                        }
                    ...
                    {
                        _logger.error("HttpClientSample failed with exception.", ee);
                    }
                }
            }
            ...
            {
                // The following code is similar to callWithHttpClient() and demonstrates how to make a call to Predix Cloud.
                // We bypass it with (byPassFlag == true) because the URL does not point to an actual Predix Cloud.
                boolean byPassFlag = true;
                if ( byPassFlag ) return;

                // Return if IPredixCloudHttpClientFactory is not injected.
                if ( getCloudHttpClient() == null ) return;
                ...
                for (int ii = 0; ii < maxTries; ii++)
                {
                    try
                    {
                        ...
                        URI uri = new URI("some/url/to/the/cloud");
     ...
     * 
     * @param clientFactory Instance of HttpClientFactory. Use this to create a new private instance of IHttpClient service.
     */
     ...
     * 
     * @param clientFactory The IHttpClientFactory to clear.
     */
    ...
    /**
     * @return the IHttpClient supports Predix Cloud authenticated communication.
     */
    ...
    public void setPredixCloudHttpClientFactory(IPredixCloudHttpClientFactory clientFactory)
    {
        ...
        {
            _logger.error("Error occured in creating authenticated HTTP client", e); //$NON-NLS-1$
        }
    }
