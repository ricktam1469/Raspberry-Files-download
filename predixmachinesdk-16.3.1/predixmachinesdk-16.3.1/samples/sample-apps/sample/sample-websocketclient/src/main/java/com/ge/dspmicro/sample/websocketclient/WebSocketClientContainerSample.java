...
package com.ge.dspmicro.sample.websocketclient;
...
public class WebSocketClientContainerSample
{
    ...
    /**
     * TEST_STRING A test string to be used in validating correct websocket transfer
     */
    static final String           TEST_STRING    = "A test string";                                              //$NON-NLS-1$
    /**
     * TEST_BYTE A test string that will be converted to a byte buffer to be used in
     * validating correct websocket transfer
     */
    static final String           TEST_BYTE      = "A test of binary data sending";                              //$NON-NLS-1$
    /**
     * TEST_BYTE_BUFF1 A test byte buffer to be used in validating correct websocket transfer
     */
    ...
    /**
     * Sets up the locat websocket session and sends some data.
     * Sessions that are created here are not explicitly closed because
     * web sockets will close automatically with a timeout. Sessions can be
     * manually closed using a session.close() call.
     * 
     * @param context The OSGi Component Context
     * @throws InterruptedException Exception thrown by the sleep function between echo tests
     */
    @Activate
        ...
        Runnable runner = new Runnable()
        {
            ...
            public void run()
            {
                try
                {
                    ...
                    locSession = getWebSocketContainer().connectToServer(WebSocketClientHandlerSample.class,
                ...
                {
                    _logger.error("Client: Failed to send messages to server", e); //$NON-NLS-1$
                }
     ...
     * 
     * @param container Websocket Container used to connect sessions with the server
     */
    @Reference
